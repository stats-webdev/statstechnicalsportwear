...
function AllProductCards({ cart, increaseQuantity, decreaseQuantity, removeAllFromCart }) {
  const [products, setProducts] = useState([]);
  const { productType, subType } = useParams(); // e.g., "Tops", "Bottoms", "CoreSeries", "WomensCore", "AllSeries"
...
const colorMapping = {
  ...
  const categoryMapping = {
    Tops: ["short sleeves", "tank tops", "blouses", "crop tops", "long sleeves"],
    Bottoms: ["jeans", "pants", "shorts", "skirts"],
    Accessories:["accessories", "hats & headwear","bags"]
  };

  // Mapping for Series
  const seriesCategoryMapping = { 
    "Core Collection": ["ACTIVE", "B.A.D", "BLADE", "RAGLAN", "RUDIMENT", "MEMBRANE", "TCG", ], 
    "Womens Core": ["CARBONITE", "DURAFLEX", "B.A.D"],
    "Special Edition":["BRUCE LEE","VINCE CARTER MERCH","MELO MERCH", "YEAR OF THE DRAGON"], 
    "School Merch":["UP REPLICA AND MERCH", "ATENEO"],
    "Tranquil Capsule":["TRANQUIL"],
    "Battlesuit Collection":["BATTLESUIT"],
    "Kobe Tribute":["KOBE TRIBUTE"],
  };
...
const filteredProducts = filterProducts(
  products.filter((product) => {
    ...
    const isTypeMatch = categoryMapping[productType]?.includes(product.Type);
    const isSeriesMatch = seriesCategoryMapping[productType]?.includes(product.Series);

    if (productType === "Womens Core" && product.Series === "B.A.D") {
      return product.Gender === "Womens";
    }
    if (productType === "School Merch") {
      if (subType === "AUTHENTIC JERSEYS") {
        return product.ID.startsWith("RUPS");
      } else if (subType === "UP MERCH") {
        return product.ID.startsWith("UPS");
      }
      return seriesCategoryMapping["School Merch"].includes(product.Series);
    }
    return isTypeMatch || isSeriesMatch;
  }),
  filters,
  chosenColor,
  selectedSize,
