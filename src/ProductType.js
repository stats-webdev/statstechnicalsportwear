...
function ProductType({ products, cart, increaseQuantity, decreaseQuantity, removeAllFromCart }) {
...
const colorMapping = {
  ...
  // Normalize and map special types
  const normalizedType = productType?.toLowerCase() || '';
  const typeMapping = {
    'shorts': ['shorts', 'jersey shorts'],
    'tank tops': ['tank tops', 'jersey tank'],
    'long sleeves': ['long sleeves', 'jersey long sleeves'],
    'short sleeves': ['short sleeves', 'jersey'],
  };
  const allowedTypes = typeMapping[normalizedType] || [normalizedType];

  const filteredProducts = filterProducts(
    products.filter(
      (product) => {
        // Keep type filtering for the route param
        const normalizedType = productType?.toLowerCase() || '';
        const typeMapping = {
          'shorts': ['shorts', 'jersey shorts'],
          'tank tops': ['tank tops', 'jersey tank'],
        ...
        const allowedTypes = typeMapping[normalizedType] || [normalizedType];
        return allowedTypes.includes(product.Type?.toLowerCase());
      }
    ),
    filters,
    chosenColor,
  ...
  const bannerObj = fetchedData?.find((item) => item[normalizedType]);
  const bannerSrc = bannerObj ? bannerObj[normalizedType] : '';
...
const availableDesignFor = Array.from(
  ...
  return (
    <div className="series-page">
      {/* banner */}
      {bannerSrc ? (
        <img src={bannerSrc} alt={`${productType} Banner`} className="series-banner" />
      ) : (
        <Banner />
      )}

      <h2 style={{ marginLeft: '5%', fontWeight: 'normal' }}>{productType}</h2>
      <hr style={{ width: '90%' }} />
